// #region debug-point H5:script-init
(()=>{try{const s='admin-login-failure';fetch('http://127.0.0.1:7777/event',{method:'POST',body:JSON.stringify({sessionId:s,runId:'pre',hypothesisId:'H5',location:'js/script.js:1',msg:'[DEBUG] script.js cargado (modulo)',data:{hasConfig:!!(typeof window!=='undefined'&&window.__SUPABASE_CONFIG__),url:window?.location?.href||''},ts:Date.now()})}).catch(()=>{})}catch{}})();
// #endregion
...
function clearLegacyLocalAuthData() {
    ...
    localStorage.removeItem('sb-piiaoucyrlvrodzxwjeh-auth-token');
}
...
async function fetchUserProfile(userId) {
    ...
    if (!client || !userId) {
        console.log('[LOGIN-DEBUG] fetchUserProfile: FALTA client o userId', { client: !!client, userId });
        return null;
    }

    console.log('[LOGIN-DEBUG] fetchUserProfile: Haciendo SELECT de profiles, userId =', userId);
    ...
    if (error) {
        console.warn('[LOGIN-DEBUG] ❌ fetchUserProfile ERROR:', error.code, error.message, error.details, error.hint);
        return null;
    }

    console.log('[LOGIN-DEBUG] ✅ fetchUserProfile EXITO, data =', JSON.stringify(data, null, 2));
    console.log('[LOGIN-DEBUG] 👉 ROLe leído =', data?.role);
    return data;
}
...
async function validateLoginForm(event) {
    ...
    console.log('[LOGIN-DEBUG] 🔐 validateLoginForm INICIADO');
    ...
    try {
        console.log('[LOGIN-DEBUG] resolveEmailForLogin para identificador =', loginIdentifier);
        const resolvedEmail = await resolveEmailForLogin(loginIdentifier);
        console.log('[LOGIN-DEBUG] Email resuelto =', resolvedEmail);
        ...
        if (error) {
            console.error('[LOGIN-DEBUG] ❌ Supabase signInWithPassword ERROR:', error.message);
            showError('email', getFriendlyAuthError(error));
            return false;
        }

        console.log('[LOGIN-DEBUG] ✅ signInWithPassword OK, user.id =', data.user.id, 'email =', data.user.email);
        console.log('[LOGIN-DEBUG] user_metadata =', JSON.stringify(data.user.user_metadata, null, 2));

        const profile = await fetchUserProfile(data.user.id);
        const localUser = buildLocalUser(data.user, profile);

        console.log('[LOGIN-DEBUG] 👤 localUser FINAL =', JSON.stringify(localUser, null, 2));
        console.log('[LOGIN-DEBUG] 🎯 ROLe detectado =', localUser.role);
        console.log('[LOGIN-DEBUG] 🎯 getPostLoginTarget =', getPostLoginTarget(localUser));
        ...
        const target = getPostLoginTarget(localUser);
        sessionStorage.setItem('postLoginTarget', target);
        console.log('[LOGIN-DEBUG] ✔ Guardado en sessionStorage: postLoginTarget =', target);
        console.log('[LOGIN-DEBUG] 🔄 Redirigiendo a loading.html...');

        window.location.href = POST_LOGIN_LOADING_PATH;
    } catch (error) {
        console.error('[LOGIN-DEBUG] ❌ Error capturado en login:', error);
        showError('email', getFriendlyAuthError(error));
    }
...
// ============================================================
// EXPOSICIÓN AL WINDOW GLOBAL (para onsubmit del HTML)
// (script.js es type="module" → sus funciones no son globales por defecto)
// ============================================================
window.validateLoginForm = validateLoginForm;
window.validateRegisterForm = validateRegisterForm;
window.loginWithGoogle = loginWithGoogle;
window.togglePasswordVisibility = togglePasswordVisibility;
window.clearLegacyLocalAuthData = clearLegacyLocalAuthData;
window.redirectIfAuthenticated = redirectIfAuthenticated;
console.log('[LOGIN-DEBUG] ✅ Funciones expuestas a window global:', Object.keys({ validateLoginForm, validateRegisterForm, loginWithGoogle, togglePasswordVisibility }));
